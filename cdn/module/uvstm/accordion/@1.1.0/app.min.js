/* 
    ...
    Version: uvstm-accordion @1.1.0
    Author: Mykhailo Unniiiverse (unniiiverse.github.io)
    License: MIT
*/

function uvstm_accordion(){const triggers=document.querySelectorAll('.uvstm-accordion-trigger');const contents=document.querySelectorAll('.uvstm-accordion-content');if(triggers&&contents){triggers.forEach((el,i)=>{if(!el.hasAttribute('data-uvstm-accordion-customID')){el.setAttribute('id',`uvstm-accordion-triggerID--${i}`);el.setAttribute('aria-controls',`uvstm-accordion-contentID--${i}`)}else{let params=el.getAttribute('data-uvstm-accordion-customID').split(' ');el.setAttribute('id',params[0]);el.setAttribute('aria-controls',params[1])}})
